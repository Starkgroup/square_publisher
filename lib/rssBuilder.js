...
export function createRssBuilder({ baseUrl, feedSize }) {
  ...
  function buildFeed() {
    ...
    const itemsXml = posts.map(p => {
      ...
      const description = escapeXml(p.summary || p.text.slice(0, 300));
      let enclosure = '';
      ...
      <description><![CDATA[${p.summary ? p.summary : p.text}]]></description>${enclosure}
    </item>`;
    }).join('\n');
...
${itemsXml}\n  </channel>`;
  ...
  return {
    ...
    invalidate() {
      cache = { xml: null, etag: null, lastModified: null, builtAt: 0 };
    },
    rebuild() {
      buildFeed();
      return cache;
