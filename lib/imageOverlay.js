...
const OVERLAY_CONFIG = {
  ...
  paddingX: 40,
  paddingY: 200, // Start below logo area
...
export async function renderOverlay({ imageBuffer, title, tag }) {
  ...
  const boxY = OVERLAY_CONFIG.paddingY;
  
  // Draw semi-transparent background with shadow
  ctx.save();
  ...
  ctx.fillStyle = OVERLAY_CONFIG.bgColor;
  ctx.beginPath();
  ctx.roundRect(boxX, boxY, boxWidth, boxHeight, OVERLAY_CONFIG.boxRadius);
  ctx.fill();
  ctx.restore();
