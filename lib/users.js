...
export function getAllUsers() {
  ...
  return db.prepare(`
    SELECT id, email, role, client_key, auto_publish_enabled, created_at, last_login_at 
    FROM users 
...
export function getUserByClientKey(clientKey) {
  const db = getDb();
  const user = db.prepare('SELECT id, email, role, client_key, auto_publish_enabled, created_at, last_login_at FROM users WHERE client_key = ?').get(clientKey);
  return user || null;
}
